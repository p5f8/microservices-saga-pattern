...
public class Sail extends AbstractSaga<SailState, SailStep> {
    ...
    public void completeFight(SailFightResultEvent event) {
        if (this.vikings.equals(event.getSurvivedVikings())) {
            this.setStep(SailStep.COMPLETED);
        } else {
            this.moveTo(SailStep.VALKIRIA_REQUESTED);
        }
    }
}
